...
class TMDB Extends Controller{
	...
	private function _call2($action, $appendToResponse){
	...
	public function getTrailer($id){
		...
		$url = self::_API_URL_.'movie/'.$id.'/videos'.'?api_key='. $this->getApikey()  ;
	...
	public function getMovie($idMovie){
		$appendToResponse = 'append_to_response=trailers,images,casts,translations';
	...
	public function displaylatestMovies()
	{
		...
		$latest=$this->getDiscoverMovie($page=1);
